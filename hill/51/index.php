...
$configAdminPass		= "5175891";										//管理员密码 注:安全起见,默认密码不能登陆管理
...
$strLenMax				= 32;											//文件名字限制长度 (防止撑破表格)
$configEachPageMax		= 8;											//每页显示的图片数目
$configEachLineMax		= 4;											//每行显示的图片数目
$configTDWidth			= 238;											//表格宽度
$configTDHeight			= 166;											//表格高度
$configPageMax			= 5;											//分页前后预览数
$configDirPasswordFile	= "neatpicPassword.php";						//密码文件
$configTilte			= "五瓣兰";							//标题
$configVer				= "1.2.3";										//程序版本号
$configAllowSizes		= 20;											//允许上传的图片容量的总大小 （单位为M）
$user					= "hill";										//定义权限
$homeUrl				= "http://hill.51.net";							//网站主页地址
...
Class neatpic
{
	...
	*/
	...
	function ShowJS()
	{
		...
		*******************************************************************************/
		...
		document.write(".cPopText { font-family: Verdana, Tahoma; background-color: #E9E0D3; border: 1px #000000 solid; font-size: 11px; padding-right: 4px; padding-left: 4px; height: 20px; padding-top: 2px; padding-bottom: 2px; filter: Alpha(Opacity=0)}");
	...
	*/
	...
	function showCSS()
	{
		...
		color:#333333; background-color:#E9E0D3
		}
		</style>
		");
	}
	...
	*/
	...
	function showTitle()
	{
		...
		print("<A NAME=\"TOP\">\n");
		print("<BR>\n");
		print("<center><IMG SRC=\"../web/51.jpg\" width=526 height=116 border=0></conter><br>\n");
		print("<br><br><br><div align=center style=\"font-size:10.5pt;color:#000000\"><b>\n");
		print($this->configTilte);
		print("</b></div></center>\n");
		print("<BR>\n");
		print("<center><font color=black>指如兰花心似莲，不笑烟花只笑禅。</font></center><BR>\n");
	}
	...
	*/
	...
	function showState()
	{
		...
		if ($this->showHomeUrl == true) {
			...
			print("[ <A HREF=\"" . $this->homeUrl . "\"><FONT COLOR=red>主站</FONT></A> ]");
			print("</td>\n");
		}
...
/*
<table width="93%" border="0" cellpadding="0" cellspacing="1" bgcolor="#E9E0D3">
	...
	function makeOptionList()
	{
		...
		if ($session['loginUser'] != null && $session['loginUser'] == $this->user)
		{
			...
			for($i = 1; $i < $this->dirNum; $i++) {
				$this->dirOptionList .= "<A HREF=\"" . $_SERVER['PHP_SELF'] . "?rmPath=" . rawurlencode($this->dirArray[$i]) . "&thisPath=" . rawurlencode($_GET['path']) . "&action=rmdirquite\" alt=\"真要删除[" . $this->dirNameArray[$i] . "]目录及文件吗？\">Del</A> [ <A HREF=\"" . $_SERVER['PHP_SELF'] . "?path=" . rawurlencode($this->dirArray[$i]) . "\" alt=\"进入[" . $this->dirNameArray[$i] . "]目录\"> " . $this->dirNameArray[$i] . "</A> ] ";
			}
		} else {
			for($i = 1; $i < $this->dirNum; $i++) {
				$this->dirOptionList .= "[ </A><A HREF=\"" . $_SERVER['PHP_SELF'] . "?path=" . rawurlencode($this->dirArray[$i]) . "\" alt=\"进入[" . $this->dirNameArray[$i] . "]目录\"> " . $this->dirNameArray[$i] . "</A> ] ";
			}
		}
		...
		else
		{
			...
			for($i = $this->start; $i < $this->offSet; $i++)
			{
				...
				*/
				...
				print("</CENTER></TD><TD bgcolor=\"#E9E0D3\" height=\"30\"><CENTER> " . $this->picRealSizeWidth . " × " . $this->picRealSizeHeight . " </CENTER></TD><TD bgcolor=\"#E9E0D3\" height=\"20\"><CENTER>" . $picFileSize . " KB</CENTER></TD></TR></TBODY></TABLE></TD>\n");
		...
		if ($_GET['action'] == 'rmdirquite') {
			...
			if ($session['loginUser'] != null && $session['loginUser'] == $this->user) 
			{
				if ($_GET['do'])
				{
					...
					print("<table width=\"1024\" align=center>\n");
					print("<tbody>\n");
					print("<tr>\n");
		...
		if ($session['loginUser'] != null && $session['loginUser'] == $this->user)
		{
			...
			if ($_GET['action'] == 'uploadmore')
			{
				...
				else
				{
					...
					print("<td bgcolor=\"#E9E0D3\" height=\"50\" style=\"border: 1px solid #CCCCCC\" align=center><BR>\n");
	...
	function showHelp()
	{
		if ($_GET['action'] == 'showhelp')
		{
			...
			print ("
				...
				<table width=750><tbody>
					<tr><td bgcolor='#E9E0D3' style='border: 1px solid #CCCCCC' align='Left'>
					<br>
			");
			...
			for($i = 1 ; $i <= count($helpContent); $i++)
			{
				print "	<tr><td bgcolor='#E9E0D3' height='25' style='border: 1px solid #CCCCCC' >";
				print "&nbsp;&nbsp;" . $i . ".<font color='blue'>".$helpContent[$i][0]."</font><a name=".$helpContent[$i][2]."></a>";
				print "</td></tr><tr><td bgcolor='#E9E0D3' height='25' style='border: 1px solid #CCCCCC' > ";
